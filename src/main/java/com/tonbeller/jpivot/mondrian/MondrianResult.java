...
public class MondrianResult extends ResultBase {
  ...
  private void initData() throws ResultLimitExceededException {
    ...
    List<MondrianCell> cells = new ArrayList<>();
    for (int i = 0; i < nCells; i++) {
      mondrian.olap.Cell monCell = monResult.getCell(iar);
      MondrianCell cell = new MondrianCell(monCell, mmodel);//注释
        /*********************改造部分*********************/
        ...
        if (false) {
            ...
            c.setValue(monCell.getFormattedValue());
            c.setFormattedValue(monCell.getFormattedValue(), formatStringParser);
        }
        addProperties(c, i);*///添加表格额外的属性
        /*********************改造部分*********************/
        cell.setFormattedValue(monCell.getFormattedValue(), formatStringParser);//注释
        //addProperties(cell, i);//添加表格额外的属性
      aCells.add(cell);
     // cells.add(cell);
      if (nCells > 1) {
    ...
    /**
    * @description 添加表格额外的属性 做预警箭头或颜色
    * @param c
    * @param i
    * @return void
    * @author rsp
    ...
    */
    public static void addProperties(MondrianCell c, int i) {
    ...
    public  void addProperties2() {
        List<MondrianCell> cells = aCells;
        Cell cell = cells.get(0).getMonCell();//measuresMembers
        String obj = cell.getDrillThroughSQL(false);
        if(obj != null && obj.indexOf("报名")!=-1){
            for (int i = 0; i < aCells.size(); i++) {

                if (aCells.size() > 3 * i + 2 && "100.00%".equals(cells.get(3 * i + 2).getFormattedValue())) {
                    Property[] props = new Property[1];
                    PropertyImpl p = new PropertyImpl();
                    props[0] = p;
                    p.setName("style");
                    p.setValue("green");
                    p.setLabel(p.getName());
                    cells.get(3 * i + 2).setProperties(props);
                    //cells.get(3*i+2).setProperties(props);
                }
                if (cells.get(i).getProperties() == null) {
                    //cells.get(i).setProperties(null);
                    Property[] props2 = new Property[1];
                    PropertyImpl p2 = new PropertyImpl();
                    props2[0] = p2;
                    p2.setLabel(p2.getName());
                    cells.get(i).setProperties(props2);
            ...
            for (int i = 0; i < aCells.size(); i++) {
                ...
                if (aCells.size() > 2 * i + 1 && !"".equals(cells.get(2 * i + 1).getFormattedValue()) && cells.get(2 * i).getFormattedValue().equals(cells.get(2 * i + 1).getFormattedValue())) {
                    Property[] props = new Property[1];
                    PropertyImpl p = new PropertyImpl();
                    props[0] = p;
                    //cells.get(2*i+1).setProperties(null);
                    p.setName("style");
                    p.setValue("green");
                    p.setLabel(p.getName());
                    cells.get(2 * i + 1).setProperties(props);
                    //cells.get(3*i+2).setProperties(props);
                }
                if (aCells.size() > 2 * i + 1 && !"".equals(cells.get(2 * i + 1).getFormattedValue()) && !"".equals(cells.get(2 * i).getFormattedValue()) && Integer.parseInt(cells.get(2 * i).getFormattedValue().replace(",", "")) < Integer.parseInt(cells.get(2 * i + 1).getFormattedValue().replace(",", ""))) {
                    Property[] props = new Property[1];
                    PropertyImpl p = new PropertyImpl();
                    props[0] = p;
                    //cells.get(2*i+1).setProperties(null);
